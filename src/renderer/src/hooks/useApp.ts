...
import type { Session, Message, MessageType, CastResult, SessionState } from '../types'
import { autoCast, deriveHexagrams, uid, formatRelative } from '../utils/casting'
...
export function useApp(): AppState & AppActions {
  ...
  async function loadSessions(): Promise<void> {
    try {
      const rows = await window.api.listSessions() as Array<Record<string, unknown>>
      const loaded: Session[] = rows.map(r => ({
        id: r.id as string,
      ...
      setSessions(loaded)
      if (loaded.length > 0) {
        await loadSessionMessages(loaded[0].id, loaded)
  ...
  async function loadSessionMessages(sessionId: string, sessionList: Session[]): Promise<void> {
    try {
  ...
  const createNewSession = useCallback(async () => {
    ...
    // Add welcome message
    const welcome = makeMessage(id, 'assistant', 'welcome',
  ...
  const setActiveSession = useCallback(async (id: string) => {
    ...
    if (session && session.messages.length === 0) {
      await loadSessionMessages(id, sessions)
    }
  }, [sessions])
  ...
  const sendQuestion = useCallback(async (question: string) => {
    ...
    updateSession(activeId, s => ({
      ...s, title: trimmed.slice(0, 40), question: trimmed, state: 'asked'
    }))
    await window.api.updateSession(activeId, { title: trimmed.slice(0, 40), question: trimmed })

    // Show method selection
    const methodMsg = makeMessage(activeId, 'assistant', 'method_select',
      'Hãy chọn phương pháp gieo quẻ:', { question: trimmed })
    addMessageToSession(activeId, methodMsg)
  }, [activeId])

  const chooseMethod = useCallback(async (method: 'auto' | 'coin') => {
    if (!activeId) return
    ...
    if (method === 'auto') {
      ...
      const resultMsg = makeMessage(activeId, 'assistant', 'hexagram_result',
        `Quẻ: ${result.primaryHex.fullName}`, { castResult: result })
      ...
      const stepMsg = makeMessage(activeId, 'assistant', 'casting_step',
        'Hào 1 (Sơ Hào) — Nhấn "Tung tiền" để gieo.', { step: 0 })
  ...
  return {
    sessions, activeSession, isLoading, coinStepIndex, pendingCoinLines,
    createNewSession, setActiveSession, deleteSession, sendQuestion, chooseMethod, tossCoinStep
  }
}
